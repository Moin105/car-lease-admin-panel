...
function Users () {
    ...
    const deleteUser = async (id) => {
        try {
          const response = await fetch(`https://leaseovername.com/api/admin/user/delete/${id}`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${bearerToken}`
            }
                        ...
                        <tbody>
                            {data.map((order, index) => (
                                <tr key={index}>
                                    ...
                                    <td><span><div><button onClick={()=>{handleRouteChange(`/user-edit/${order.id}`,order.id)}}>Edit</button><button onClick={()=>{deleteUser(order.id)}}>Delete</button></div></span></td>
                                </tr>
                            ))}
                        </tbody>
                    : null}
